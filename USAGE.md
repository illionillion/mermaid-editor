...
新機能として、既存のMermaidコードからフローチャートを生成できます：

**インポート手順**

1. 左パネルの「**インポート**」ボタンをクリック
2. **シンタックスハイライト付きエディタ**が表示されます
3. 既存のMermaidコードを貼り付けまたは入力
4. 「**インポート**」ボタンでフローチャートを生成
...
- **🔄 自動レイアウト**: インポートしたノードが階層的に自動配置
- **🔍 エラー検出**: 不正なコードの場合はエラーメッセージを表示
- **📱 モバイル対応**: スマホでもタッチ操作で快適に利用可能
...
**Q: モバイルでエッジからノードが作成できない**
A: ハンドルを長めにタッチし、空の場所まで正確にドラッグ

**Q: コードが生成されない**
A: ノードとエッジが正しく接続されているか確認

**Q: インポートできない**
A: Mermaidコードの記法が正しいか確認。対応していない記法の場合はエラーメッセージを参照
